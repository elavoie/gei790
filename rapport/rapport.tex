...
% Alex
% 0.5 page
Ce rapport présente les résultats de recherche pour l'amélioration de l'intelligence d'un joueur informatisé(\textit{JI}).
Les \textit{JI} qui évoluent entourés de joueurs humains font face à un problème critique. En fait, ils ne possèdent pas une intelligence suffisante pour s'adapter aux multiples situations imprévisibles des humains. De plus, les \textit{JI} sont trop prévisibles et rendent les jeux rapidement monotones. À ce jour, l'amélioration de \textit{JI} est l'élément le plus crucial pour une évolution saine de la compagnie.

Pour répondre à ce problème, l'ingénieur en chef a déterminé l'utilisation de méthodes d'intelligence artificiel. Les recherches effectuées comportent plusieurs volets afin de s'assurer de répondre aux besoins réels de la majorité de nos jeux à plateforme multijoueurs. Voici les études qui ont été réalisées:

\textbf{Méthode de recherches}
\begin{enumerate}
 \item Non-informée
 \item Informée avec heuristique
\end{enumerate}
...
Avec ces résultats nous pouvons voir la performance, en temps, de chaque algorithme différent. Il ne faut pas comparer les résultats entres les environnements car ils n'ont en fait aucune lien ensemble. 

Le plremier cas à réaliser est que la technique de profondeur d'abord est vraiment risqué. Cette technique a dépasser la mémoire maximale permise à l'environnement prolog. Cette technique est donc à éviter. Il semble que l'algorithme s'étant trop dans une branch dans l'environnement 3 et 4.
Le deuxième cas à réaliser est que l'algorithme A* est toujours pire que toutes les autres algorithme. En théorie le A* devrait être rapide car il élimine 
