...
Avec ces résultats nous pouvons voir l'évolutions des algorithmes. Nous avons doubler la distance entre le jouer et son but pour chaque environnement. Nous pouvons ainsi voir un estimé de la complexité de chacune des algorithmes.

En premier considération, la technique de profondeur d'abord est une algorithme vraiment risqué. Cette technique dépasse rapidement la mémoire maximale permise à l'environnement prolog. Cette technique est donc à éviter. Il semble que l'algorithme s'étant trop dans une branche dans l'environnement 2, 3 et 4.

En second considération, la technique A* s'embale plus le but s'éloigne. Nous pouvons voir que le A* effectue beaucoup plus de calcul que n'importe qu'elle autre solution. L'algorithme utilise une queue avec priorité tout comme la méthode Avare ainsi que la méthode à coût uniforme. Cependant, elle tient compte de plus de paramètre que ces derniers; elle tient compte de chacun des paramètres utilisé par ces derniers. Le fait d'utilisé deux éléments dans le coût total pour l'insertion dans la queue fait que beaucoup de changement de branche est effectuer.

En troisième considération, lorsque le but est très loins, seul l'algorithme greedy réussi à résoudre le problème.

Avec tous ces considération, nous suggérons l'utilisation de la méthode greedy.
